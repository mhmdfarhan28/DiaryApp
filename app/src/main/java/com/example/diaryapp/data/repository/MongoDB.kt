...
object MongoDB: MongoRepository {
    ...
    override fun getFilteredDiaries(zonedDateTime: ZonedDateTime): Flow<Diaries> {
        return if (user !=null) {
            try {
                realm.query<Diary>(query = "ownerId = $0 AND date >= $1 AND date <= $2",
                    user.id,
                    RealmInstant.from(zonedDateTime.toLocalDate().atStartOfDay(ZoneId.systemDefault()).toInstant().epochSecond, 0),
                    RealmInstant.from(zonedDateTime.toLocalDate().atStartOfDay(ZoneId.systemDefault()).plusHours(24).toInstant().epochSecond, 0),
                ).asFlow().map { result ->
                    RequestState.Success(
                        data = result.list.groupBy {
                            it.date.toInstant()
